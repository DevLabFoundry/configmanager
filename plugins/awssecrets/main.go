...
func StrategyTokenValue(tokenPtr, tokenLen, metadataStrPtr, metadataStrLen, outPtr, outCap, outLenPtr uint32) int32 {
	defer func() {
	...
	if metadataStrLen > 0 {
		metadataStrBytes := bytesFromPtrLen(metadataStrPtr, metadataStrLen)
		if !utf8.Valid(metadataStrBytes) {
			if outLenPtr != 0 {
				if lenCell := bytesFromPtrLen(metadataStrPtr, 4); len(lenCell) == 4 {
					binary.LittleEndian.PutUint32(lenCell, uint32(len(tokenBytes)))
				}
			}
			return plugins.ERR_INVALID_UTF8
	...
	inStr := string(tokenBytes)
	outStr := "TOKEN_VALUE:" + strings.ToUpper(inStr)
...
// scaffolds the `_instantiate` method
func main() {}

// GOOS=wasip1 GOARCH=wasm go build -buildmode=c-shared -o awssecrets.wasm
